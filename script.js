...
const MAP_LIST = ["Ascent","Breeze","Lotus","Split","Pearl","Haven","Fracture","Bind","Abyss","Corrode","Sunset","Icebox"];
const AGENT_LIST = [
...
function decodeShare(str) {
    try {
        if (str.startsWith("v2")) {
            ...
            if (labelStr) labelStr.split("|").forEach((l, i) => { if (i < 5 && l) playerLabels[i] = decodeURIComponent(l); });
            const bannedMaps = [];
            for (let i = 0; i < bannedStr.length; i++) { const m = MAP_LIST[parseInt(bannedStr[i], 36)]; if (m) bannedMaps.push(m); }
            return { comps, playerLabels, bannedMaps };
        }
    } catch(e) {}
...
function showVetoStinger(onDone) {
    ...
    logoWrapA.style.borderColor = vTColor("A") + "55";
    logoWrapA.appendChild(makeLogo("A", 96));
    ...
    logoWrapB.style.borderColor = vTColor("B") + "55";
    logoWrapB.appendChild(makeLogo("B", 96));
    ...
    sideB.appendChild(logoWrapB);
    sideB.appendChild(nameB);

    const flash = document.createElement("div");
    flash.classList.add("vstg-flash");
    ...
    // Impact flash at ~900ms
    setTimeout(() => stinger.classList.add("vstg-impact"), 900);

    // At 2600ms: start stinger exit AND build broadcast simultaneously → seamless crossfade
    setTimeout(() => {
        stinger.classList.add("vstg-exit");
        onDone(); // broadcast screen starts fading in while stinger fades out
        setTimeout(() => stinger.remove(), 640);
    }, 2600);
}
