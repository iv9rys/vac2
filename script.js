...
{ const _ap = teamProfiles[activeProfileIdx]; teamName = _ap.teamName || ""; teamLogo = _ap.teamLogo || null; playerLabels = _ap.playerLabels || ["P1","P2","P3","P4","P5"]; }
...
function saveActiveProfile() {
    ...
    teamProfiles[activeProfileIdx] = { ...teamProfiles[activeProfileIdx], teamName, teamLogo, playerLabels: [...playerLabels] };
    localStorage.setItem("teamProfiles", JSON.stringify(teamProfiles));
...
function renderSettings() {
    ...
    if (activeSettingsTab === "My Team") {
        ...
        teamProfiles.forEach((profile, idx) => {
            ...
            pillName.textContent = profile.profileName || `Profile ${idx + 1}`;
        ...
        newProfBtn.addEventListener("click", () => {
            saveActiveProfile();
            teamProfiles.push({ id: Date.now(), profileName: "New Profile", teamName: "", teamLogo: null, playerLabels: ["P1","P2","P3","P4","P5"] });
            activeProfileIdx = teamProfiles.length - 1;
            teamName = ""; teamLogo = null; playerLabels = ["P1","P2","P3","P4","P5"];
            localStorage.setItem("teamProfiles", JSON.stringify(teamProfiles));
            localStorage.setItem("activeProfileIdx", String(activeProfileIdx));
            renderSettings();
        });
        ...
        const profNameLabel = document.createElement("div");
        profNameLabel.classList.add("profile-field-label");
        profNameLabel.textContent = "Profile Name";
        const profNameIn = document.createElement("input");
        profNameIn.type = "text";
        profNameIn.classList.add("team-name-input");
        profNameIn.value = teamProfiles[activeProfileIdx]?.profileName || "";
        profNameIn.placeholder = "e.g. Main Roster";
        profNameIn.addEventListener("input", e => {
            if (teamProfiles[activeProfileIdx]) {
                teamProfiles[activeProfileIdx].profileName = e.target.value;
                localStorage.setItem("teamProfiles", JSON.stringify(teamProfiles));
            }
        });
        profileSection.appendChild(profNameLabel);
        profileSection.appendChild(profNameIn);
...
function renderScrimPlannerTab(container) {
    ...
    addBtn.textContent = "+ Session";

    const dlBtn = document.createElement("button");
    dlBtn.id = "scrim-dl-btn";
    dlBtn.classList.add("download-btn");
    dlBtn.textContent = "Download Schedule";
    ...
    addBtn.addEventListener("click", () => {
        ...
        openScrimSessionModal(null, defaultDay, session => {
...
function buildWeekGrid(container, dates) {
    container.innerHTML = "";
    const grid = document.createElement("div");
    grid.classList.add("scrim-grid");
    ...
    dates.forEach((date, di) => {
        ...
        col.classList.add("scrim-day-col");
        if (date.getTime() === todayMs) col.classList.add("today");

        const dayHdr = document.createElement("div");
        dayHdr.classList.add("scrim-day-header");
        const dayName = document.createElement("div");
        dayName.classList.add("scrim-day-name");
        dayName.textContent = DAY_NAMES_SHORT[di];
        const dayNum = document.createElement("div");
        dayNum.classList.add("scrim-day-num");
        if (date.getTime() === todayMs) dayNum.classList.add("today");
        dayNum.textContent = date.getDate();
        dayHdr.appendChild(dayName);
        dayHdr.appendChild(dayNum);
        col.appendChild(dayHdr);

        const dayBody = document.createElement("div");
        dayBody.classList.add("scrim-day-body");
        ...
        daySessions.forEach(session => {
            const card = buildScrimSessionCard(session);
            card.addEventListener("click", () => {
                openScrimSessionModal(session, session.day,
                    updated => {
                        const idx = scrimPlan.sessions.findIndex(s => s.id === session.id);
                        if (idx >= 0) scrimPlan.sessions[idx] = updated;
                        saveScrimPlan();
                        buildWeekGrid(container, dates);
                    },
                    () => {
        ...
        addDayBtn.classList.add("scrim-day-add-btn");
        addDayBtn.textContent = "+";
        addDayBtn.title = `Add scrim on ${DAY_NAMES_SHORT[di]}`;
        addDayBtn.addEventListener("click", () => {
            openScrimSessionModal(null, di, session => {
                scrimPlan.sessions.push(session);
                saveScrimPlan();
                buildWeekGrid(container, dates);
            }, null);
        });
        dayBody.appendChild(addDayBtn);
...
function buildScrimSessionCard(session) {
    const card = document.createElement("div");
    card.classList.add("scrim-session-card");
    card.style.background = MAP_COLORS[session.map] || "#1a2a3a";

    if (session.map) {
        const bg = document.createElement("div");
        bg.classList.add("scrim-session-bg");
        bg.style.backgroundImage = `url('maps/${session.map.toLowerCase()}.jpg')`;
        card.appendChild(bg);
    }

    const overlay = document.createElement("div");
    overlay.classList.add("scrim-session-overlay");
    card.appendChild(overlay);

    const content = document.createElement("div");
    content.classList.add("scrim-session-content");

    const mapEl = document.createElement("div");
    mapEl.classList.add("scrim-session-map");
    mapEl.textContent = session.map || "—";
    ...
    if (timeStr) {
        ...
        timeEl.classList.add("scrim-session-time");
    ...
    if (session.opponent) {
        ...
        oppEl.classList.add("scrim-session-opponent");
    ...
    if (session.notes) {
        ...
        notesEl.classList.add("scrim-session-notes");
...
function openScrimSessionModal(existingSession, defaultDay, onSave, onDelete) {
    ...
    const mtitle = document.createElement("div");   mtitle.classList.add("scrim-modal-title"); mtitle.textContent = isEdit ? "Edit Scrim" : "Add Scrim";
    ...
    const saveBtn2 = document.createElement("button"); saveBtn2.classList.add("scrim-modal-save"); saveBtn2.textContent = isEdit ? "Save Changes" : "Add Scrim";
...
async function downloadScrimPlanner() {
    ...
    const dates    = scrimWeekDates(scrimPlan.weekOffset);
    const sessions = scrimPlan.sessions || [];
    ...
    const cardGap  = 8;
    const colGap   = 10;
    const colW     = Math.floor((W - marginX * 2 - colGap * 6) / 7);
    const maxCards = Math.max(...byDay.map(d => d.length), 1);
    const contentH = dayHdrH + maxCards * (cardH + cardGap) + 16;
    const totalH   = headerH + 20 + contentH + footerH + 12;
    ...
    const C = lightMode ? {
        bg:        "#f6f6f4",
        colBg:     "#eaeae8",
        dayName:   "rgba(0,0,0,0.4)",
        dayNum:    "rgba(0,0,0,0.78)",
        ...
        bg:        "#0d0d0d",
        colBg:     "#141414",
        dayName:   "rgba(255,255,255,0.35)",
        dayNum:    "rgba(255,255,255,0.82)",
        weekSub:   "rgba(255,255,255,0.3)",
    ...
    ctx.textAlign = "center";
    ctx.fillText(scrimWeekLabel(dates), W / 2, headerH + 13);

    const gridY = headerH + 28;

    dates.forEach((date, di) => {
        const x = marginX + di * (colW + colGap);

        ctx.fillStyle = C.colBg;
        canvasFillRounded(ctx, x, gridY, colW, contentH, 10);

        ctx.fillStyle = C.dayName;
        ctx.font = "700 10px Arial";
        ctx.textAlign = "center";
        ctx.fillText(DAY_NAMES_SHORT[di].toUpperCase(), x + colW / 2, gridY + 20);

        ctx.fillStyle = C.dayNum;
        ctx.font = '900 20px "Archivo Black", Arial';
        ctx.fillText(date.getDate(), x + colW / 2, gridY + 50);

        byDay[di].forEach((session, si) => {
            const cy = gridY + dayHdrH + si * (cardH + cardGap);
            const cx = x + 6;
            const cw = colW - 12;

            ctx.fillStyle = MAP_COLORS[session.map] || "#1a1a2a";
            canvasFillRounded(ctx, cx, cy, cw, cardH, 8);

            if (mapImgs[session.map]) {
                ctx.save();
                ctx.globalAlpha = 0.38;
                ctx.beginPath();
                canvasRoundedPath(ctx, cx, cy, cw, cardH, 8);
                ctx.clip();
                ctx.drawImage(mapImgs[session.map], cx, cy, cw, cardH);
                ctx.restore();
            }
            ...
            ctx.fillStyle = "white";
            ctx.font = '900 12px "Archivo Black", Arial';
            ctx.textAlign = "left";
            ctx.fillText(session.map || "—", cx + 9, cy + cardH - 34);

            const tStr = session.endTime ? `${session.startTime} – ${session.endTime}` : (session.startTime || "");
            ctx.fillStyle = "rgba(255,255,255,0.72)";
            ctx.font = "500 10px Arial";
            ctx.fillText(tStr, cx + 9, cy + cardH - 20);

            if (session.opponent) {
                ctx.fillStyle = "rgba(255,255,255,0.52)";
                ctx.font = "500 10px Arial";
                ctx.fillText(`vs ${session.opponent}`, cx + 9, cy + cardH - 7);
            }
        });
    });

    const footerY = gridY + contentH + 10;
    ctx.fillStyle = C.sep;
    ctx.fillRect(marginX, footerY, W - marginX * 2, 1);
    ctx.fillStyle = C.footerVAC;
    ctx.font = '900 14px "Archivo Black", Arial';
    ctx.textAlign = "center";
    ctx.fillText("VAC", W / 2, footerY + 24);
    ctx.fillStyle = C.footerSub;
    ctx.font = "500 10px Arial";
    ctx.fillText("VALORANT · AGENT · COMPS", W / 2, footerY + 38);
    ctx.textAlign = "left";
    ctx.fillText("@iv9rys", marginX, footerY + 38);

    const link = document.createElement("a");
    link.download = `VAC-scrim-${dates[0].toISOString().slice(0, 10)}.png`;
    link.href = canvas.toDataURL("image/png");
    link.click();

    if (btn) { btn.textContent = "Download Schedule"; btn.disabled = false; }
}
