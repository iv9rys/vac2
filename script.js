...
function renderSettings() {
    ...
    if (activeSettingsTab === "My Team") {
        ...
        // ── Tracker.gg Import ─────────────────────────────────
        const trackerSection = makeSection("Import from Tracker.gg");
        ...
        trackerBtn.addEventListener("click", async () => {
            ...
            const match = url.match(/tracker\.gg\/valorant\/premier\/teams\/([0-9a-f-]{36})/i);
            if (!match) {
            ...
            try {
                const res = await fetch(`https://api.tracker.gg/api/v2/valorant/premier/teams/${teamId}`, {
                ...
                // Team name
                const importedName = data.name || data.teamName || "";
                if (importedName) {
                ...
                // Team logo
                const logoUrl = data.logoUrl || data.iconUrl || data.icon?.url || data.image || "";
                if (logoUrl) {
                    await new Promise(resolve => {
                        const img = new Image();
                        img.crossOrigin = "anonymous";
                ...
                // First 5 player names (IGNs)
                const members = data.members || data.roster || data.players || [];
                if (members.length > 0) {
                    members.slice(0, 5).forEach((m, i) => {
                        const ign = (m.name || m.platformInfo?.platformUserHandle || m.playerName || m.summonerName || "").split("#")[0];
                        if (ign) {
                            const raw = ign.slice(0, 6).toUpperCase();
                            playerLabels[i] = raw;
                        }
                    });
                    localStorage.setItem("playerLabels", JSON.stringify(playerLabels));
                ...
                trackerMsg.textContent = `Imported "${teamName}" successfully!`;
                trackerMsg.className = "tracker-msg tracker-msg-success";
                renderSettings();
            } catch (err) {
                trackerMsg.textContent = "Failed to import — check the link or try again.";
                trackerMsg.className = "tracker-msg tracker-msg-error";
        ...
        trackerSection.appendChild(trackerRow);
        trackerSection.appendChild(trackerMsg);
