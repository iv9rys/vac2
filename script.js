...
function openVetoSettings(onApply) {
    ...
    const selectedMaps = new Set(vetoMaps || ALL_MAPS);
...
function showVetoAnnounce(announceEl, type, map, team, gridEl, onDone) {
    ...
    function dismiss(side) {
        announceEl.style.transition = "opacity 0.52s cubic-bezier(0.4,0,1,1), transform 0.52s cubic-bezier(0.4,0,1,1)";
        announceEl.style.opacity = "0";
        announceEl.style.transform = "translateY(-16px) scale(0.96)";
        setTimeout(() => {
