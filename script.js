...
async function downloadComfortList() {
    ...
    ctx.font = "500 12px Arial";
    ctx.textAlign = "right";
    ctx.fillText("COMFORT LIST", W - marginX, 54);
    ctx.fillStyle = "rgba(255,255,255,0.07)";
    ctx.fillRect(marginX, 78, W - marginX * 2, 1);
...
function renderComfortList() {
    ...
    const allAgents = Object.values(agentsByClass).flat();
    ...
    function getTiers() {
        return JSON.parse(localStorage.getItem("comfortTiers")) || {};
    }

    function saveTiers(t) {
        localStorage.setItem("comfortTiers", JSON.stringify(t));
    }

    function makeCard(agent, fromTierKey) {
        const card = document.createElement("div");
        card.classList.add("comfort-card");
        card.draggable = true;
        card.innerHTML = `
            <img src="agents/${agent}.png" alt="${capitalize(agent)}">
            <div class="comfort-card-name">${capitalize(agent)}</div>
        `;
        card.addEventListener("dragstart", e => {
            e.dataTransfer.setData("agent", agent);
            e.dataTransfer.setData("fromTier", fromTierKey || "");
    ...
    function makeDropZone(el, onDrop) {
        ...
        el.addEventListener("drop", e => {
            ...
            onDrop(e.dataTransfer.getData("agent"), e.dataTransfer.getData("fromTier"));
        });
    }
    ...
    const assigned = new Set(Object.values(tiers).flat());
    ...
    TIERS.forEach(({ key, color }) => {
        ...
        label.classList.add("comfort-tier-label");
        label.style.backgroundColor = color;
        label.textContent = key;

        const agentsEl = document.createElement("div");
        agentsEl.classList.add("comfort-tier-agents");

        (tiers[key] || []).forEach(agent => agentsEl.appendChild(makeCard(agent, key)));

        makeDropZone(agentsEl, (agent, fromTier) => {
            const t = getTiers();
            if (fromTier) t[fromTier] = (t[fromTier] || []).filter(a => a !== agent);
            if (!t[key]) t[key] = [];
            if (!t[key].includes(agent)) t[key].push(agent);
            saveTiers(t);
            renderComfortList();
        });
    ...
    pool.classList.add("comfort-pool");

    const poolTitle = document.createElement("div");
    poolTitle.classList.add("comfort-pool-title");
    poolTitle.textContent = "Agents";
    pool.appendChild(poolTitle);

    const poolAgents = document.createElement("div");
    poolAgents.classList.add("comfort-pool-agents");
    unassigned.forEach(agent => poolAgents.appendChild(makeCard(agent, null)));

    makeDropZone(poolAgents, (agent, fromTier) => {
        if (!fromTier) return;
        const t = getTiers();
        t[fromTier] = (t[fromTier] || []).filter(a => a !== agent);
        saveTiers(t);
        renderComfortList();
    });

    pool.appendChild(poolAgents);
