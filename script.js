...
function renderBuilder() {
    ...
    const vlrBtn = document.getElementById("vlr-import-btn");
    if (vlrBtn) vlrBtn.onclick = openVlrImportModal;
...
// ── Import pro comps from a VLR.gg team page ─────────────────
function openVlrImportModal() {
    const overlay = document.createElement("div");
    overlay.classList.add("scrim-modal-overlay");
    ...
    const mhdr   = document.createElement("div");   mhdr.classList.add("scrim-modal-header");
    const mtitle = document.createElement("div");   mtitle.classList.add("scrim-modal-title");
    mtitle.textContent = "Import from VLR.gg";
    const mclose = document.createElement("button"); mclose.classList.add("scrim-modal-close"); mclose.innerHTML = "&#x2715;";
    mhdr.appendChild(mtitle); mhdr.appendChild(mclose);
    modal.appendChild(mhdr);
    ...
    const inSection = document.createElement("div"); inSection.classList.add("scrim-modal-section");
    const inLabel   = document.createElement("div"); inLabel.classList.add("scrim-modal-label"); inLabel.textContent = "Team URL";
    inSection.appendChild(inLabel);
    const teamInput = document.createElement("input");
    teamInput.type = "text";
    teamInput.classList.add("vlr-import-input");
    teamInput.placeholder = "https://www.vlr.gg/team/2/sentinels";
    inSection.appendChild(teamInput);
    mbody.appendChild(inSection);

    const status = document.createElement("div"); status.classList.add("vlr-import-status");
    mbody.appendChild(status);

    const results = document.createElement("div"); results.classList.add("vlr-import-results");
    mbody.appendChild(results);
    modal.appendChild(mbody);

    const mfoot = document.createElement("div"); mfoot.classList.add("scrim-modal-footer");
    const actionBtn = document.createElement("button");
    actionBtn.classList.add("scrim-modal-save");
    actionBtn.textContent = "Fetch Comps";
    mfoot.appendChild(actionBtn);
    modal.appendChild(mfoot);
    ...
    let fetchedMaps = null; // { mapName: { agents, opponent } }

    actionBtn.addEventListener("click", async () => {
        if (fetchedMaps) {
            // Phase 2: apply the fetched comps
            Object.entries(fetchedMaps).forEach(([mapName, data]) => {
                const canonical = allMaps.find(m => m.toLowerCase() === mapName.toLowerCase()) || mapName;
                comps[canonical] = {};
                data.agents.forEach((agent, i) => { comps[canonical][i + 1] = agent; });
            });
            saveComps();
            renderBuilder();
            closeModal();
        ...
        if (!url) { status.textContent = "Paste a VLR.gg team link first."; status.classList.add("error"); return; }

        status.classList.remove("error");
        status.textContent = "Fetching comps from VLR.gg…";
        results.innerHTML = "";
        actionBtn.disabled = true;
        actionBtn.textContent = "Fetching…";

        try {
            const res  = await fetch(`/api/vlr-comps?team=${encodeURIComponent(url)}`);
            const data = await res.json();
            if (!res.ok) throw new Error(data.error || "Failed to fetch comps");

            fetchedMaps = data.maps;
            status.classList.remove("error");
            status.textContent = `${data.team} — ${Object.keys(fetchedMaps).length} map${Object.keys(fetchedMaps).length === 1 ? "" : "s"} found`;

            Object.entries(fetchedMaps).forEach(([mapName, mapData]) => {
                const row = document.createElement("div");
                row.classList.add("vlr-import-map-row");
                const name = document.createElement("span"); name.classList.add("map-name"); name.textContent = mapName;
                const agents = document.createElement("span"); agents.classList.add("agents");
                agents.textContent = mapData.agents.map(capitalize).join(", ");
                row.appendChild(name); row.appendChild(agents);
                results.appendChild(row);
            });

            actionBtn.disabled = false;
            actionBtn.textContent = `Import ${Object.keys(fetchedMaps).length} Comp${Object.keys(fetchedMaps).length === 1 ? "" : "s"}`;
        } catch (err) {
