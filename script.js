...
function vetoTeamLabel(team) {
    return team === "A" ? (teamName || "Your Team") : (vetoTeamBName || "Opponent");
}
...
function renderVetoTab(container) {
    ...
    ["Bo1", "Bo3", "Bo5"].forEach(fmt => {
        ...
        btn.addEventListener("click", () => {
            ...
            teamANameEl.textContent = teamName || "Your Team";
            buildGrid();
            updateStatus();
            buildStepBar();
            updateResult();
    ...
    cardA.classList.add("veto-team-card", "veto-team-card-a");
    const teamANameEl = document.createElement("div");
    teamANameEl.classList.add("veto-team-card-name");
    teamANameEl.textContent = teamName || "Your Team";
    const teamASubEl = document.createElement("div");
    teamASubEl.classList.add("veto-team-card-sub");
    teamASubEl.textContent = "Team A";
    cardA.appendChild(teamANameEl);
    cardA.appendChild(teamASubEl);
    ...
    cardB.classList.add("veto-team-card", "veto-team-card-b");
    const teamBInput = document.createElement("input");
    teamBInput.classList.add("veto-team-b-input");
    teamBInput.placeholder = "Opponent";
    teamBInput.value = vetoTeamBName !== "Opponent" ? vetoTeamBName : "";
    teamBInput.maxLength = 24;
    teamBInput.addEventListener("input", e => {
        vetoTeamBName = e.target.value || "Opponent";
        updateStatus();
        updateResult();
    });
    const teamBSubEl = document.createElement("div");
    teamBSubEl.classList.add("veto-team-card-sub");
    teamBSubEl.textContent = "Team B";
    cardB.appendChild(teamBInput);
    cardB.appendChild(teamBSubEl);
    cardB.addEventListener("click", () => teamBInput.focus());
    ...
    // ── Grid: build once, update via classes ─────────────────────
    function buildGrid() {
        gridEl.innerHTML = "";
        mapPool.forEach(map => {
            const card = document.createElement("div");
            ...
            const nameEl = document.createElement("div");
            nameEl.classList.add("veto-map-name");
            ...
            card.addEventListener("click", () => {
                if (vetoComplete || vetoBannedBy[map] || vetoPickedBy[map] || vetoDecider === map) return;
                const seq = getVetoSequence();
                const step = seq[vetoStep];
                if (!step) return;
                if (step.action === "ban") vetoBannedBy[map] = step.team;
                else vetoPickedBy[map] = step.team;
                vetoStep++;
                if (vetoStep >= seq.length) {
                    const remaining = mapPool.filter(m => !vetoBannedBy[m] && !vetoPickedBy[m]);
    ...
    function updateAllCards() {
        gridEl.querySelectorAll(".veto-map-card").forEach(card => {
            ...
            const taken = !!(bannedBy || pickedBy || isDecider);
            ...
            const slAction = card.querySelector(".veto-sl-action");
            const slTeam   = card.querySelector(".veto-sl-team");
            if (slAction && slTeam) {
                if (bannedBy)       { slAction.textContent = "BANNED";  slTeam.textContent = "by " + vetoTeamLabel(bannedBy); }
                else if (pickedBy)  { slAction.textContent = "PICKED";  slTeam.textContent = "by " + vetoTeamLabel(pickedBy); }
                else if (isDecider) { slAction.textContent = "DECIDER"; slTeam.textContent = ""; }
                else                { slAction.textContent = "";         slTeam.textContent = ""; }
            }
        });
    }
    ...
    function updateStatus() {
        ...
        if (!team) return;
        const color = team === "A" ? accentColor : "#ff4655";
        accentBar.style.background = color;

        const nameEl2 = document.createElement("span");
        nameEl2.classList.add("veto-status-team-name");
        nameEl2.style.color = color;
        nameEl2.textContent = vetoTeamLabel(team);
        ...
        actionEl.classList.add("veto-status-action-text");
        actionEl.textContent = action === "ban" ? "Select a map to ban" : "Select a map to pick";

        const stepEl = document.createElement("span");
        stepEl.classList.add("veto-status-step");
        stepEl.textContent = `Step ${vetoStep + 1} of ${seq.length}`;

        statusInner.appendChild(nameEl2);
        statusInner.appendChild(sep);
        statusInner.appendChild(actionEl);
        statusInner.appendChild(stepEl);
    }
    ...
    function updateStepBar() {
        stepsEl.querySelectorAll(".veto-step-seg").forEach((seg, idx) => {
            ...
            if (idx < vetoStep || vetoComplete) {
                ...
                seg.querySelector(".veto-step-pill").style.background = "";
                seg.style.color = "";
            } else if (idx === vetoStep && !vetoComplete) {
                seg.classList.add("current");
                const { team } = seq[idx] || {};
                const color = team === "A" ? accentColor : "#ff4655";
                seg.querySelector(".veto-step-pill").style.background = color;
                seg.querySelector(".veto-step-pill").style.boxShadow = `0 0 6px ${color}`;
                seg.querySelector(".veto-step-lbl").style.color = color;
            } else {
                seg.querySelector(".veto-step-pill").style.background = "";
                seg.querySelector(".veto-step-lbl").style.color = "";
                seg.style.color = "";
            }
        });
    }
    ...
    function updateResult() {
        ...
        hdr.classList.add("veto-result-header");
        hdr.textContent = "Veto Summary";
        res.appendChild(hdr);
        const body = document.createElement("div");
        body.classList.add("veto-result-body");
        res.appendChild(body);

        const tA = vetoTeamLabel("A"), tB = vetoTeamLabel("B");

        const addBanRows = (maps, team) => {
            maps.forEach(map => {
                const row = document.createElement("div");
                row.classList.add("veto-result-row", "is-ban");
                const lbl = document.createElement("span");
                lbl.classList.add("veto-result-label");
                lbl.textContent = (team === "A" ? tA : tB) + " Ban";
                const mapName = document.createElement("span");
                mapName.classList.add("veto-result-map");
                mapName.textContent = map;
                row.appendChild(lbl);
                row.appendChild(mapName);
                body.appendChild(row);
            });
        };

        const addPickRow = (map, team, cls) => {
            const row = document.createElement("div");
            row.classList.add("veto-result-row", cls);
            const lbl = document.createElement("span");
            lbl.classList.add("veto-result-label");
            lbl.textContent = (team === "A" ? tA : tB) + " Pick";
            const mapName = document.createElement("span");
            mapName.classList.add("veto-result-map");
            mapName.textContent = map;
            row.appendChild(lbl);
        ...
        const addDeciderRow = (map) => {
            const row = document.createElement("div");
            row.classList.add("veto-result-row", "is-decider");
            const lbl = document.createElement("span");
            lbl.classList.add("veto-result-label");
            lbl.textContent = "Decider";
            const mapName = document.createElement("span");
            mapName.classList.add("veto-result-map");
            mapName.textContent = map;
            row.appendChild(lbl);
        ...
        const addSideBtns = (row, map) => {
            const btns = document.createElement("div");
            btns.classList.add("veto-side-btns");
            const atkBtn = document.createElement("button");
            atkBtn.classList.add("veto-side-btn", "atk");
            atkBtn.textContent = "ATK";
            if (vetoSides[map] === "atk") atkBtn.classList.add("selected");
            const defBtn = document.createElement("button");
            defBtn.classList.add("veto-side-btn", "def");
            defBtn.textContent = "DEF";
            if (vetoSides[map] === "def") defBtn.classList.add("selected");
            atkBtn.addEventListener("click", () => {
                vetoSides[map] = vetoSides[map] === "atk" ? null : "atk";
                atkBtn.classList.toggle("selected", vetoSides[map] === "atk");
                defBtn.classList.remove("selected");
            });
            defBtn.addEventListener("click", () => {
                vetoSides[map] = vetoSides[map] === "def" ? null : "def";
                defBtn.classList.toggle("selected", vetoSides[map] === "def");
                atkBtn.classList.remove("selected");
            });
        ...
        // Interleave picks/bans in order added (object insertion order)
        const tA_picks = picksA; const tB_picks = picksB;
        addBanRows(bansA, "A");
        addBanRows(bansB, "B");
        tA_picks.forEach(m => addPickRow(m, "A", "is-pick-a"));
        tB_picks.forEach(m => addPickRow(m, "B", "is-pick-b"));
        if (vetoDecider) addDeciderRow(vetoDecider);
