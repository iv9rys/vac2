...
function normalizeTeamLogo(source) {
    if (!source) return null;
    const markers = ["/Premier logos/", "/premier-logos/", "Premier logos/", "premier-logos/"];
    for (const marker of markers) {
        const markerIndex = source.indexOf(marker);
        if (markerIndex >= 0) return `Premier logos/${source.slice(markerIndex + marker.length)}`;
    }
    return source;
}
teamLogo = normalizeTeamLogo(teamLogo);
...
if (activeProfileIdx >= teamProfiles.length) activeProfileIdx = 0;
{
    ...
    teamName = _ap.teamName || ""; teamLogo = normalizeTeamLogo(_ap.teamLogo); playerLabels = _ap.playerLabels || ["P1","P2","P3","P4","P5"];
    if (_ap.teamLogo !== teamLogo) { _ap.teamLogo = teamLogo; localStorage.setItem("teamProfiles", JSON.stringify(teamProfiles)); }
...
function openCrestMaker() {
    ...
    overlay.innerHTML = `
        <div class="premier-logo-modal" role="dialog" aria-modal="true" aria-label="Premier logo changer">
            ...
            <div class="premier-logo-body">
                <div class="premier-logo-library" data-panel="select"><div class="premier-logo-toolbar"><span>Choose a Premier mark</span><span class="premier-logo-count">49 logos</span><button type="button" class="download-btn premier-continue" disabled>Continue to color</button></div><div class="premier-logo-grid"></div><div class="premier-dropzone"><strong>Drop an image here</strong><span>or import a PNG, JPG, or WebP</span><button type="button" class="download-btn premier-import-btn">Import image</button><input class="premier-import-input" type="file" accept="image/*" hidden></div></div>
                <div class="premier-logo-editor" data-panel="edit" hidden><div class="premier-editor-toolbar"><button type="button" class="premier-back">Back to logos</button><span>Customize colors</span></div><div class="premier-editor-preview"><canvas width="300" height="300"></canvas></div><div class="premier-editor-controls"><div class="premier-color-heading">Choose a layer to recolor</div><div class="premier-color-roles"><button type="button" class="premier-color-role active" data-role="primary"><span class="premier-role-dot"></span><span>Primary</span><strong class="premier-role-value">#DD3B41</strong></button><button type="button" class="premier-color-role" data-role="secondary"><span class="premier-role-dot"></span><span>Secondary</span><strong class="premier-role-value">#EDC657</strong></button><button type="button" class="premier-color-role" data-role="border"><span class="premier-role-dot"></span><span>Border</span><strong class="premier-role-value">#343236</strong></button></div><div class="premier-color-heading">Premier palette</div><div class="premier-color-swatches"></div><label>Selected layer color<input class="premier-active-color-input" type="color" value="#dd3b41"></label><label>Background<input class="premier-bg-input" type="color" value="#343236"></label><label class="premier-check"><input class="premier-bg-toggle" type="checkbox"> Show background</label><button type="button" class="download-btn premier-use-created">Use this logo</button></div></div>
            </div>
        </div>`;
    ...
    const continueBtn = overlay.querySelector(".premier-continue");
    const editor = overlay.querySelector('[data-panel="edit"]');
    const library = overlay.querySelector('[data-panel="select"]');
    const roleInputs = {
        primary: { input: document.createElement("input"), color: "#dd3b41" },
        secondary: { input: document.createElement("input"), color: "#edc657" },
        border: { input: document.createElement("input"), color: "#343236" }
    };
    const primaryInput = roleInputs.primary.input;
    const secondaryInput = roleInputs.secondary.input;
    const borderInput = roleInputs.border.input;
    let activeColorRole = "primary";
    const activeColorInput = overlay.querySelector(".premier-active-color-input");
    const swatchRow = overlay.querySelector(".premier-color-swatches");
    const logoColors = [
        "#343236", "#6e6b6e", "#5f5d5f", "#acaaab", "#dedcd8",
        "#690910", "#dd3b41", "#bd3532", "#8d4116", "#ec772c",
        "#916336", "#edc657", "#c1a54b", "#949121", "#edf271",
        "#755e2e", "#b8b53d", "#a49837", "#748325", "#b3cf5b",
        "#6f9144", "#a9f153", "#8cc759", "#07645d", "#079f74",
        "#b0b8b0", "#dbebfa", "#216291", "#3ac3b2", "#bdfcfb",
        "#4d2bb2", "#906ff4", "#805ada", "#9a2ff0", "#cb8bf9",
        "#4d3264", "#77568f", "#7b4784", "#d44eb8", "#ff8ef1",
        "#55364a", "#ba8cb3", "#a06f8c", "#c95172", "#d489a4",
        "#5e3e4c", "#efbcd5", "#d89bbb", "#e85f7e", "#f7b7c6"
    ];
    function showEditor() { library.hidden = true; editor.hidden = false; drawEditor(); }
    function showLibrary() { editor.hidden = true; library.hidden = false; }
    const roleButtons = overlay.querySelectorAll(".premier-color-role");
    function syncRoleUI() {
        const active = roleInputs[activeColorRole];
        activeColorInput.value = active.color;
        roleButtons.forEach(button => {
            const role = button.dataset.role;
            button.classList.toggle("active", role === activeColorRole);
            button.querySelector(".premier-role-value").textContent = roleInputs[role].color.toUpperCase();
            button.querySelector(".premier-role-dot").style.background = roleInputs[role].color;
            roleInputs[role].input.value = roleInputs[role].color;
        });
        swatchRow.querySelectorAll(".premier-color-swatch").forEach(swatch => swatch.classList.toggle("active", swatch.dataset.color === active.color));
    }
    roleButtons.forEach(button => button.addEventListener("click", () => { activeColorRole = button.dataset.role; syncRoleUI(); }));
    logoColors.forEach(color => {
        const swatch = document.createElement("button");
        swatch.type = "button";
        swatch.className = "premier-color-swatch";
        swatch.dataset.color = color;
        swatch.style.background = color;
        swatch.title = color;
        swatch.addEventListener("click", () => { roleInputs[activeColorRole].color = color; syncRoleUI(); drawEditor(); });
    ...
    PREMIER_LOGOS.forEach(name => {
        ...
        button.addEventListener("click", () => { selectedName = name; selectedSource = `Premier logos/Premier_Team_Logo_${name}.webp`; grid.querySelectorAll(".premier-logo-choice").forEach(item => item.classList.remove("active")); button.classList.add("active"); continueBtn.disabled = false; });
        grid.appendChild(button);
    });
    grid.firstElementChild.classList.add("active");
    continueBtn.disabled = false;
    continueBtn.addEventListener("click", showEditor);
    overlay.querySelector(".premier-back").addEventListener("click", showLibrary);
    const importInput = overlay.querySelector(".premier-import-input");
    const readImage = file => {
        if (!file || !file.type.startsWith("image/")) return;
        const reader = new FileReader();
        reader.onload = event => { selectedName = "Imported logo"; selectedSource = event.target.result; continueBtn.disabled = false; showEditor(); };
        reader.readAsDataURL(file);
    };
    ...
    function drawEditor() {
        ...
        image.onload = () => {
            ...
            if (overlay.querySelector(".premier-bg-toggle").checked) { context.fillStyle = overlay.querySelector(".premier-bg-input").value; context.fillRect(0, 0, canvas.width, canvas.height); }
            const scale = Math.min(240 / image.width, 240 / image.height);
            ...
            const primary = hexToRgbArray(primaryInput.value), secondary = hexToRgbArray(secondaryInput.value), border = hexToRgbArray(borderInput.value);
            let minBrightness = 255, maxBrightness = 0;
            for (let i = 0; i < pixels.data.length; i += 4) {
                if (pixels.data[i + 3] === 0) continue;
                const brightness = (pixels.data[i] * 0.299) + (pixels.data[i + 1] * 0.587) + (pixels.data[i + 2] * 0.114);
                minBrightness = Math.min(minBrightness, brightness);
                maxBrightness = Math.max(maxBrightness, brightness);
            }
            const brightnessRange = Math.max(1, maxBrightness - minBrightness);
            const secondaryCutoff = minBrightness + brightnessRange * 0.42;
            const primaryCutoff = minBrightness + brightnessRange * 0.74;
            for (let i = 0; i < pixels.data.length; i += 4) {
                const alpha = pixels.data[i + 3];
                if (alpha === 0) continue;
                const brightness = (pixels.data[i] * 0.299) + (pixels.data[i + 1] * 0.587) + (pixels.data[i + 2] * 0.114);
                const color = brightness >= primaryCutoff ? primary : brightness >= secondaryCutoff ? secondary : border;
    ...
    activeColorInput.addEventListener("input", () => { roleInputs[activeColorRole].color = activeColorInput.value; syncRoleUI(); drawEditor(); });
    [".premier-bg-input", ".premier-bg-toggle"].forEach(selector => overlay.querySelector(selector).addEventListener("input", drawEditor));
