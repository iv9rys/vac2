...
function renderSettings() {
    ...
    if (activeSettingsTab === "My Team") {
        ...
        // ── Tracker.gg import toggle (below player names) ─────
        const trackerToggleBtn = document.createElement("button");
        trackerToggleBtn.classList.add("tracker-toggle-btn");
        trackerToggleBtn.textContent = "↗ Import from Tracker.gg";
        labelsSection.appendChild(trackerToggleBtn);
        ...
        trackerInput.classList.add("team-name-input");
        trackerInput.placeholder = "https://tracker.gg/valorant/premier/teams/...";
        const trackerRow = document.createElement("div");
        trackerRow.classList.add("tracker-import-row");
        const trackerBtn = document.createElement("button");
        trackerBtn.classList.add("download-btn");
        trackerBtn.textContent = "Import";
        const trackerMsg = document.createElement("span");
        trackerMsg.classList.add("tracker-msg");
        trackerRow.appendChild(trackerInput);
        trackerRow.appendChild(trackerBtn);
        trackerBox.appendChild(trackerRow);
        trackerBox.appendChild(trackerMsg);
        labelsSection.appendChild(trackerBox);

        trackerToggleBtn.addEventListener("click", () => {
            const open = trackerBox.classList.toggle("visible");
            trackerToggleBtn.textContent = open ? "✕ Close" : "↗ Import from Tracker.gg";
            if (open) trackerInput.focus();
        });

        trackerBtn.addEventListener("click", async () => {
            const url = trackerInput.value.trim();
            const idMatch = url.match(/tracker\.gg\/valorant\/premier\/teams\/([0-9a-f-]{36})/i);
            if (!idMatch) {
                trackerMsg.textContent = "Invalid link — must contain /valorant/premier/teams/";
                trackerMsg.className = "tracker-msg tracker-msg-error";
                return;
            }
            const teamId = idMatch[1];
            trackerBtn.disabled = true;
            trackerBtn.textContent = "Importing...";
            trackerMsg.textContent = "";
            trackerMsg.className = "tracker-msg";
            try {
                // Try proxies in order until one succeeds
                const apiUrl = `https://api.tracker.gg/api/v2/valorant/premier/teams/${teamId}`;
                const pageUrl = `https://tracker.gg/valorant/premier/teams/${teamId}`;
                const proxies = [
                    { url: `https://corsproxy.io/?url=${encodeURIComponent(apiUrl)}`, type: "json" },
                    { url: `https://corsproxy.io/?url=${encodeURIComponent(pageUrl)}`, type: "html" },
                ...
                for (const proxy of proxies) {
                    try {
                        ...
                        if (proxy.type === "json") {
                            ...
                            const rawHtml = proxy.type === "allorigins"
                                ? ((await res.json()).contents || "")
                                : await res.text();
                            if (!rawHtml) continue;
                            const ndMatch = rawHtml.match(/<script id="__NEXT_DATA__"[^>]*>([\s\S]*?)<\/script>/);
                            if (!ndMatch) continue;
                            const nd = JSON.parse(ndMatch[1]);
                            const pp = nd?.props?.pageProps || {};
                            team = pp.team || pp.data?.team || pp.data || null;
                        }
                        if (team) break;
                    } catch (_) { lastErr = _.message; }
                }
                if (!team) throw new Error(lastErr);

                // Team name
                const importedName = team.name || team.teamName || "";
                if (importedName) {
                    teamName = importedName;
                    localStorage.setItem("teamName", teamName);
                }

                // Team logo
                const logoUrl = team.logoUrl || team.iconUrl || team.logo || team.icon?.url || team.imageUrl || "";
                if (logoUrl) {
                    await new Promise(resolve => {
                        const img = new Image();
                        img.crossOrigin = "anonymous";
                        img.onload = () => {
                            ...
                            try {
                                teamLogo = cv.toDataURL("image/png");
                                localStorage.setItem("teamLogo", teamLogo);
                            } catch (_) {}
                            resolve();
                        };
                ...
                const members = team.members || team.roster || team.players || [];
                members.slice(0, 5).forEach((m, i) => {
                    const raw = m.name || m.playerName || m.summonerName ||
                                m.platformInfo?.platformUserHandle || m.handle || "";
                    const ign = raw.split("#")[0].trim().slice(0, 6).toUpperCase();
                    if (ign) { playerLabels[i] = ign; }
                });
                if (members.length > 0) localStorage.setItem("playerLabels", JSON.stringify(playerLabels));

                trackerMsg.textContent = `Imported "${teamName || teamId}" successfully!`;
                trackerMsg.className = "tracker-msg tracker-msg-success";
                renderSettings();
            } catch (err) {
                trackerMsg.textContent = `Failed to import — ${err.message}`;
                trackerMsg.className = "tracker-msg tracker-msg-error";
