...
function resetVeto() {
    ...
    vetoDecider = null; vetoComplete = false;
}
...
function renderVetoTab(container) {
    ...
    ["Bo1", "Bo3", "Bo5"].forEach(fmt => {
        ...
        btn.addEventListener("click", () => { vetoFormat = fmt; resetVeto(); rerender(); });
        fmtBar.appendChild(btn);
    });
    const resetBtn = document.createElement("button");
    resetBtn.classList.add("veto-reset-btn");
    resetBtn.textContent = "Reset";
    resetBtn.addEventListener("click", () => { resetVeto(); rerender(); });
    ...
    teamALabel.classList.add("veto-team-label", "veto-team-a");
    teamALabel.textContent = teamName || "Your Team";
    const vsLabel = document.createElement("div");
    vsLabel.classList.add("veto-vs");
    vsLabel.textContent = "VS";
    const teamBInput = document.createElement("input");
    teamBInput.classList.add("veto-team-input");
    ...
    teamRow.appendChild(teamALabel);
    teamRow.appendChild(vsLabel);
    teamRow.appendChild(teamBInput);
    ...
    function buildStatusHTML() {
        ...
        if (vetoComplete) return `<span class="veto-status-done">Veto complete</span>`;
        const { team, action } = seq[vetoStep] || {};
        if (!team) return "";
        const tCls = team === "A" ? "veto-status-a" : "veto-status-b";
        const act  = action === "ban" ? "Ban a map" : "Pick a map";
        return `<span class="${tCls}">${vetoTeamLabel(team)}</span><span class="veto-status-action">: ${act}</span><span class="veto-status-sub"> (${vetoStep + 1}/${seq.length})</span>`;
    }
    ...
    function refresh() {
        ...
        mapPool.forEach(map => {
            ...
            if (bannedBy)         card.classList.add("veto-banned");
            else if (pickedBy === "A") card.classList.add("veto-pick-a");
            else if (pickedBy === "B") card.classList.add("veto-pick-b");
            else if (isDecider)   card.classList.add("veto-decider");
            if (taken) card.classList.add("veto-taken");

            if (taken) {
                let badgeText = "";
                if (bannedBy)    badgeText = "BAN · " + vetoTeamLabel(bannedBy).toUpperCase();
                else if (pickedBy) badgeText = "PICK · " + vetoTeamLabel(pickedBy).toUpperCase();
                else if (isDecider) badgeText = "DECIDER";
                const badge = document.createElement("div");
                badge.classList.add("veto-map-badge");
            ...
            bg.style.background = `linear-gradient(155deg, ${MAP_COLORS[map] || "#1a1a1a"}, #040406)`;
            card.appendChild(bg);
        ...
        if (bansA.length || bansB.length || picksA.length || picksB.length || vetoDecider) {
            const res = document.createElement("div");
            res.classList.add("veto-result");
            const addRow = (label, maps, cls) => {
                if (!maps.length) return;
                const row = document.createElement("div");
                row.classList.add("veto-result-row");
                const lbl = document.createElement("span");
                lbl.classList.add("veto-result-label");
                lbl.textContent = label;
                const tags = document.createElement("span");
                tags.classList.add("veto-result-maps");
                maps.forEach(m => {
                    const tag = document.createElement("span");
                    tag.classList.add("veto-result-tag", cls);
                    tag.textContent = m;
            ...
            addRow(tA + " Bans",  bansA,  "veto-tag-ban");
            addRow(tB + " Bans",  bansB,  "veto-tag-ban");
            addRow(tA + " Pick",  picksA, "veto-tag-pick-a");
            addRow(tB + " Pick",  picksB, "veto-tag-pick-b");
            if (vetoDecider) addRow("Decider", [vetoDecider], "veto-tag-decider");
            resultEl.appendChild(res);
        }
    }

    function rerender() {
        // Refresh format buttons
        wrap.querySelectorAll(".veto-fmt-btn").forEach(b => {
            b.classList.toggle("active", b.textContent === vetoFormat);
        });
        teamALabel.textContent = teamName || "Your Team";
        refresh();
    }
