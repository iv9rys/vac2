...
function renderBuilder() {
    ...
    if (banSelect) {
        if (multipleBans) {
            ...
            banSelect.setAttribute("multiple", "");
            banSelect.innerHTML = "";
            mapPool.forEach(map => {
            ...
            banSelect.addEventListener("change", () => {
                const selected = Array.from(banSelect.selectedOptions).map(o => o.value);
                if (selected.length >= mapPool.length) {
                    // Revert to previous valid state
                    Array.from(banSelect.options).forEach(opt => {
                        opt.selected = bannedMaps.includes(opt.value);
                    });
                    showBanError();
                } else {
                    bannedMaps = selected;
